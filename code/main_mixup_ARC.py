...
model_log_dir=os.path.join(valid_name,'ARC_mixup',calib_cost_type+"_"+str(calib_cost_index)+"_lamda_"+str(args.lamda)+"_bins_"+bins_list_dir,proposed_cost_over_mixup,args.dataset,args.model_net+"_drop"+str(args.dropout)+"_mixupcoeff"+str(args.mixup_coeff))
...
for ep in range(num_epochs):
	...
	for idx,((x1,t1),(x2,t2)) in enumerate(zip(data_train1,data_train2)):
		...
		if args.cost_over_mix_image:
			SSE = net.cost_CONF_mixup_interpolated(out,t1,t2,lam) 
			COST = NNL+SSE
			COST.backward()
			...
			CE_loss+=NNL.data
			SSE_loss+=SSE.data
		else: # can be done more efficient with enough GPU memory
			NNL.backward()
			CE_loss+=NNL.data
			cost = NNL.data
			out=net.forward(x1)
			SSE = net.cost_CONF(out,t1) 
			SSE.backward()
			SSE_loss+=SSE.data
			cost+=SSE.data

			out=net.forward(x2)
			SSE = net.cost_CONF(out,t2)
			SSE.backward()
			SSE_loss+=SSE.data
			cost+=SSE.data
